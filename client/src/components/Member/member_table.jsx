...
import EditButton from './edit_button_member';
import EditForm from './edit_member_form';



const EmployeeTable = ({ id }) => {
  ...
  useEffect(() => {
    fetch("http://localhost:9797/api/members")
      .then((res) => res.json())
      .then(
        (result) => {
          ...
          {employeeList.map((val, key) => {
            return (
              <tr className='sm1:text-xs lg:text-sm'>
                ...
                <td className='boder px-4 py-2'>{val.memberID}</td>
                <td className='boder px-4 py-2'>{val.nameMember}</td>
                <td className='boder px-4 py-2'>{val.operation}</td>
                <td className='boder px-4 py-2'>{val.office}</td>
                <td className='boder px-4 py-2'>{val.statusMember === 1 ? "inactive" : "active"}</td>
